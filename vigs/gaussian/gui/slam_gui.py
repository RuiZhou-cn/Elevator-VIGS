...
class SLAM_GUI:
    ...
    def init_widget(self):
        self.window_w, self.window_h = 1360, 480
        self.window_x, self.window_y = 960+40-400, int((480*2-200)//2*1.3)+100+30  # starting position (pixels from top-left)

        self.window = gui.Application.instance.create_window("GS Viewer", self.window_w, self.window_h, self.window_x, self.window_y)
        ...
        self.behind_slider.double_value = 0.14   # default "from behind" distance passed to Frustum.update_pose
        ...
        self.scaling_slider.set_limits(0.001, 2.0)
    ...
    def _on_key(self, event):
        if event.key == gui.KeyName.P and event.type == gui.KeyEvent.DOWN:
            self.panel_visible = not self.panel_visible
            self.panel.visible = self.panel_visible
    ...
    def _on_layout(self, layout_context):
        ...
        panel_width = contentRect.width - int(self.window.size.width * 0.7)
    ...
    def _on_kf_window_chbox(self, is_checked):
        ...
        for key in self.kf_window.keys():
            for kf_idx in self.kf_window[key]:
                ...
                edge_cnt += 1
                if "keyframe_{:05d}".format(int(key)) not in self.frustum_dict.keys():
                    continue
                test1 = self.frustum_dict["keyframe_{:05d}".format(int(key))].view_dir[1]
                kf = self.frustum_dict["keyframe_{:05d}".format(int(kf_idx))].view_dir[1]
                points = [test1, kf]
                lines = [[0, 1]]
                colors = [[0, 1, 0]]
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.keyframe is not None:
            name = "keyframe_{:05d}".format(int(gaussian_packet.keyframe.uid))
            frustum = self.add_camera(
        ...
        if gaussian_packet.keyframes is not None:
            for keyframe in gaussian_packet.keyframes:
                name = "keyframe_{:05d}".format(int(keyframe.uid))
        ...
        if gaussian_packet.gtdepth is not None:
            ...
            max_depth_value = 1.1*np.percentile(depth, 95)
            depth = imgviz.depth2rgb(
                depth, min_value=0.1, max_value=max_depth_value, colormap="jet"
            )
    ...
    def get_current_cam(self):
        ...
        image_gui = torch.zeros(
            (1, int(self.window.size.height), int(self.widget3d.frame.width))
        )
    ...
    def render_o3d_image(self, results, current_cam):
        if self.depth_chbox.checked:
            ...
            opacity = 1.0 - results["transmittance"]   # the renderer returns transmittance, not opacity
