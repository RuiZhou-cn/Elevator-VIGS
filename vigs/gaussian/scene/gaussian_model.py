...
class GaussianModel:
    def __init__(self, sh_degree: int, config=None):
        ...
        self.kf_ids_version = 0   # bumped on every rewrite of unique_kfIDs (utils/elevator_submap.py caches on it)
    ...
    def rescale(self, s: float):
        """Rescale gaussian translations and radii by a scalar s.

        - xyz (3D means) are multiplied by s
        - radii/scales are multiplied by s as well
        """
        ...
        # Scale radii: apply scaling in activation space to remain numerically stable
        try:
            current_scale = self.get_scaling
            new_scale = current_scale * float(s)
            self._scaling[:] = self.scaling_inverse_activation(new_scale)
        except Exception:
            # Fallback: if anything goes wrong, do additive in log space
            self._scaling += torch.log(torch.as_tensor(float(s), device=self._scaling.device, dtype=self._scaling.dtype))
    ...
    def create_pcd_from_image(self, cam_info, init=False, depthmap=None):
        cam = cam_info
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        # when mask too many depth pixel, np.median(depth) will be 0, so set a minimum 0.005, for better visualization
        if "adaptive_pointsize" in self.config["Dataset"]:
            if self.config["Dataset"]["adaptive_pointsize"]:
                depth_np = np.asarray(depth)
                depth_np = depth_np[depth_np > 0.01]
                point_size = max(min(0.05, point_size * np.median(depth_np)), 0.005)
        ...
        return fused_point_cloud, features, scales, rots, opacities, fused_color
    ...
    def extend_from_pcd_seq(
        ...
        fused_point_cloud, features, scales, rots, opacities, _ = (
            self.create_pcd_from_image(cam_info, init, depthmap=depthmap)
        )
